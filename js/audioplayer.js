...
const audioPlayer = (function () {
    ...
    function createSliders(tuneID) {
        ...
        // flag to control restart of paused audio
        let temporarilyPaused = false;

        audioSlider.noUiSlider.on("start", function () {
            // if audio is playing, pause, while adjusting position.
            if (OneAudioPlayer.paused == false) {
                OneAudioPlayer.pause();
                temporarilyPaused = true; // set the flag
            }
        });
        
        audioSlider.noUiSlider.on("end", function () {
            // if audio was paused because of adjusting slider position
            if (temporarilyPaused == true) {    
                OneAudioPlayer.play(); // restart the audio once the slider is repositioned
                temporarilyPaused = false; // reset the flag
            }
        });
        ...
        // add the function that this slider calls on change
        speedSlider.noUiSlider.on("update", function (value) {
